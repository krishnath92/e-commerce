...
<fieldset><legend> Choisissez une couleur </legend>
<table>
	<tr>
		<td> La couleur : </td> 
		<td> <input name='couleur' type='text' placeholder='ex: Noir, Bleu...' /> </td>
	</tr>
...
";
if (isset($_GET['couleur']) AND !empty($_GET['couleur'])) {
    $couleur = $_GET['couleur'];
    $recupArticle = $db->prepare("SELECT * FROM products WHERE couleur = ?");
	$recupArticle->execute(array($couleur));
    
    $res = $db->prepare('SELECT count(*) from products where couleur = ? ');
    $res->execute(array($couleur));
    ...
    if($recupArticle->rowCount() != 0){
        ...
        echo"<table border = '1' cellpadding='4' cellspacing ='4'>" ?>
                ...
                <?= "<tr> 
                <h1 style='text-align: center;'> Les articles $couleur : ".  $nbrArticle . " articles trouvés.</h1>
                <th>ID</th>
                <th>"?> 
                ...
                <?= "</th>
                <th>"?> 
                <a href = "sousCat.php" > Sous-catégorie</a>
            <?= "</th>
                <th>Description</th>
                <th>Couleur</th>
