...
<fieldset><legend> Choisissez une catégoire </legend>
<table>
    <tr>
        <td> Catégorie : </td> 
	...
	<tr>
		<td style='margin-top:15px;'> <input name='res' type='reset' value='Annuler'/></td> 
		<td style='margin-top:15px;'> <input name='ok' type='submit' value='Valider'/></td>
	</tr><br>
</table>
</fieldset>
</form>
";
if (isset($_GET['sexe']) AND !empty($_GET['sexe'])) {
    if(isset($_GET['ok'])){
        $sexe = $_GET['sexe'];
        $recupArticle = $db->prepare("SELECT * FROM products WHERE categorie = ?");
        $recupArticle->execute(array($sexe));

        $res = $db->prepare('SELECT count(*) from products where categorie = ? ');
        $res->execute(array($sexe));
        ...
        if($recupArticle->rowCount() > 0){
            ...
            echo"<table border = '1' cellpadding='4' cellspacing ='4'>" ?>
                ...
                <?= "<tr> 
                    <h1 style='text-align: center;'> Les articles pour $sexe : ".  $nbrArticle . " articles trouvés. </h1> 
                    <th>ID</th>
                    <th>"?> 
...
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">
    <link rel="stylesheet" href="../styleminiDesign.css" >
