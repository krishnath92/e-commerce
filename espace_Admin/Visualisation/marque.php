...
<fieldset><legend> Choisissez une marque </legend>
<table>
	<tr>
		<td> La couleur : </td> 
		<td> <input name='marque' type='text' placeholder='ex: Nike, Addidas...' /> </td>
	</tr>
...
if (isset($_GET['marque']) AND !empty($_GET['marque'])) {
    $marque = $_GET['marque'];
    $recupArticle = $db->prepare("SELECT * FROM products WHERE marque = ?");
	$recupArticle->execute(array($marque));
    
    $res = $db->prepare('SELECT count(*) from products where marque = ? ');
    ...
    if($recupArticle->rowCount() != 0){
        ...
        echo"<table border = '1' cellpadding='4' cellspacing ='4'>" ?>
                ...
                <?= "<tr> 
                <h1 style='text-align: center;'> Les articles $marque : ".  $nbrArticle . " articles trouvés.</h1>
                <th>ID</th>
                <th>"?> 
