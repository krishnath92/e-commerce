...
<fieldset><legend> Entrez une quantité </legend>
<table>
	<tr>
		<td> Nombre stock: </td> 
		<td> <input name='quantité' type='number' placeholder='ex: 5' /> </td>
	</tr>
...
";
if (isset($_GET['quantité']) AND !empty($_GET['quantité'])) {
    if(isset($_GET['ok'])){
        $quantité = $_GET['quantité'];
        $recupArticle = $db->prepare("SELECT * FROM products WHERE nombre_stock = ?");
        $recupArticle->execute(array($quantité));
        ...
        if($recupArticle->rowCount() > 0){
            ...
            echo"<table border = '1' cellpadding='4' cellspacing ='4'>" ?>
                ...
                <?= "<tr> 
                    <h1 style='text-align:center;'> Les articles de quantité : $quantité</h1>
                    <th>ID</th>
                    <th>"?> 
            ...
            echo "</table>";
                ...
                echo"Aucun article ne possède cette quantité";
            }
